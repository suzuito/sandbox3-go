...
const upseartArticleSearchIndices = `-- name: UpseartArticleSearchIndices :exec
WITH t1 AS (
  SELECT
...
func (q *Queries) UpseartArticleSearchIndices(ctx context.Context) error {
	_, err := q.db.Exec(ctx, upseartArticleSearchIndices)
	return err
}
